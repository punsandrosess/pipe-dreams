...
**Botanical brand refresh.** Both tools have been migrated from the original black-and-gold palette to a botanical color system: forest backgrounds, green accents, and warm neutral text. The visual identity now matches the punsandrosess brand guide.

**Light/dark mode.** A sun/moon toggle in the header of each tool lets you switch between a dark theme (forest-on-dark, the default) and a light theme (white cards, dark text, muted green accents). Your preference saves automatically and persists across sessions.

**PDF export.** A dedicated PDF button in the header triggers the browser's native print dialog, pre-configured for clean output. For Mist Opportunities, the export renders all three tabs (Opportunities, Pipeline, and Change Log) into a single document. For Pipeline Fitness, the export renders both the Funnel and Trends tabs together, with all stage diagnostics fully expanded.
...
- PDF export that captures all three tabs in a single print-ready document
...
- PDF export with all diagnostics fully expanded, both tabs rendered, ready for stakeholder review
...
5. Toggle light/dark mode with the sun/moon button in the header
6. Click the PDF button to export a clean, full-content report
